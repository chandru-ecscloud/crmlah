...
import ECS_Broucher from "../../assets/ECS_Broucher.png";
import ECSCloud from "../../assets/ECS Cloud Corporate Deck.pdf";

const validationSchema = yup.object({
  subject: yup.string().required("*Subject is required"),
  // files: yup.array().min(1, "Attacment is Must").required("Attacment is Must"),
});
function SendCompanyProfile({ accountData }) {
  ...
  useEffect(() => {
    const fetchPDFs = async () => {
      const files = [ECS_Broucher, ECSCloud]; 
      const base64Files = await Promise.all(files.map(async (filePath) => {
        try {
      ...
      setBase64PDFs(base64Files.filter(Boolean)); // Filter out any failed loads
    };
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        formData.append("to", "sakthiveljayabal23@gmail.com");
        formData.append("from", "sakthiveljayabal23@gmail.com");
        formData.append("subject", values.subject);
        formData.append("htmlContent", values.htmlContent);
        formData.append("bodyContent ", "hii");
        base64PDFs.forEach((base64, index) => {
