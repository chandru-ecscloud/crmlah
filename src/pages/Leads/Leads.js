...
const Lead = () => {
  ...
  const filterFieldsForSpecificCompanyIds = (data) =>
    data
      .filter((row) => row.companyId === 43 || row.companyId === 53 || row.companyId === 54 || row.companyId === 55)
      .map((row, index) => {
        const {
          Location,
          Course,
  ...
  const removeDuplicatesForSpecificCompanyIds = (data, key) => {
    const seen = new Set();
    return data
      .filter((item) => item.companyId === 43 || item.companyId === 53 || item.companyId === 54 || item.companyId === 55)
      .filter((item) => {
        const value = item[key];
        if (seen.has(value)) {
          return false;
